...
public class Main {
    public static void main(String[] args) {
        ...
        System.out.println("\033[1mClient: \033[0m" + theWantedClient.name);
        System.out.printf("%24s|%20s|%30s|%30s|%34s|\n","\033[1mProduct",   "Quantity",   "Standard Unit Price",  "Promotional Unit Price",   "Line Total\033[0m");
        ...
        for (int i=1;i< parts.length;i++)
        {
            ...
            {
                System.out.printf("%20s|%20s|%30s|%30s|%30s|\n",theWantedProduct.name,productQuantity[1],"EUR "+theWantedProduct.getStandartPrice(),
                        "EUR "+theWantedProduct.getAfterPromotionPrice(),"EUR "+sumForTheProduct);
            }
            else {
                System.out.printf("%20s|%20s|%30s|%30s|%30s|\n",theWantedProduct.name,productQuantity[1],"EUR "+theWantedProduct.getStandartPrice(),
                        " ","EUR "+sumForTheProduct);//omitting the unnecessary value
            }
        }
        System.out.printf("%-40s%20s\n","\033[1mTotal Before Client Discounts: \033[0m","EUR "+sumForTheProducts);
        ...
        {
            System.out.printf("%-41s%20s\n","\033[1mBasic Client Discount:\033[0m ","EUR "+theWantedClient.personalDiscountSum);
        }
        if(theWantedClient.getAfterPersonalDiscount()!=theWantedClient.finalSumOfMoneyToBePay()) //in case there is additional discount
        {
            if(theWantedClient.getAfterPersonalDiscount()>30000) {
                System.out.printf("%-40s%20s\n","\033[1mAdditional Volume Discount:\033[0m at "+theWantedClient.above30000 , "EUR "+theWantedClient.getTotalDiscountQ());
            }
            else if(theWantedClient.getAfterPersonalDiscount()>10000)
            {
                System.out.printf("%-40s%20s\n","\033[1mAdditional Volume Discount:\033[0m at "+theWantedClient.above10000 , "EUR "+theWantedClient.getTotalDiscountQ());
            }
        }
        System.out.printf("%-41s%20s\n","\033[1mOrder Total Amount:\033[0m ","EUR "+theWantedClient.finalSumOfMoneyToBePay());
